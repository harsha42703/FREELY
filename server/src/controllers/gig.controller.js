...
const getGig = async (request, response) => {
    ...
    // console.log(_id);
    try {
        const gig = await Gig.find({_id:"643a7eabfaf8f6420161c82e"});
        // const gig = await Gig.findById(_id).populate('userID', 'username cover email description isSeller _id image');
       console.log(gig);
        if (!gig) {
